...
extension UIView {
    ...
    */
    ...
    private func showToast(toast: UIView, duration: Double, position: AnyObject) {
        ...
        if HRToastHidesOnTap {
            let tapRecognizer = UITapGestureRecognizer(target: toast, action: #selector(UIView.handleToastTapped(recognizer:)))
        ...
        UIView.animate(withDuration: HRToastFadeDuration,
            ...
            completion: { (finished: Bool) in
                let timer = Timer.scheduledTimer(timeInterval: duration, target: self, selector: #selector(UIView.toastTimerDidFinish(timer:)), userInfo: toast, repeats: false)
    ...
    private func makeToastActivity(position pos: AnyObject, message msg: String = "") {
        ...
        let activityView = UIView(frame: CGRect(x: 0.0, y: 0.0, width: HRToastActivityWidth, height: HRToastActivityHeight))
        ...
        if (!msg.isEmpty){
            ...
            let activityMessageLabel = UILabel(frame: CGRect(x: activityView.bounds.origin.x, y: (activityIndicatorView.frame.origin.y + activityIndicatorView.frame.size.height + 10), width: activityView.bounds.size.width, height: 20.0))
    ...
    */
    ...
    @objc func handleToastTapped(recognizer: UITapGestureRecognizer) {
        if let timer = objc_getAssociatedObject(self, &HRToastTimer) as? Timer { // Pankaj Modification to make it process safe
            timer.invalidate()
